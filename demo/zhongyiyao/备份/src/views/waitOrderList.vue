<template>
	...
	123
		<Diamond v-for="(item,index) in orderList" :key="index" style="height:auto;box-shadow:0px 2px 2px #ddd;">
            <span slot="tit">
                {{ item.name }}
                <router-link to="/platform/caseDetail" :style="{float:'right','padding-right':'10px',color:'#40b542'}">查看更多 >></router-link>
            </span>
            <div slot="con"  style="margin-top:20px;">
                <Row>
                    <Col class="v-line" span="3" :style="{'text-align':'left'}">主治大夫：{{ item.buyerName }}</Col>
                    <Col class="v-line" span="3">病症：{{ item.illness }}</Col>
                    ...
                    <Col class="v-line last" span="3">状态：{{ item.orderStatus }}</Col>
                </Row>
            </div>
        </Diamond>
	...
	export default{
		...
		methods:{
			init(){
                ...
                // 获取其他病历(订单——卖家)信息
                axios.get('/orders/getWaitOrdersNum', {
                	params:param
                }).then((response)=>{
